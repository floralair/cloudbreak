...
public interface SecurityGroupEndpoint {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    SecurityGroupJson getPrivate(@PathParam("name") String name);

    @GET
    @Path("account/securitygroups/{name}")
    @Produces(MediaType.APPLICATION_JSON)
    SecurityGroupJson getPublic(@PathParam("name") String name);

    @GET
    @Path("securitygroups/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    SecurityGroupJson get(@PathParam("id") Long id);

    @DELETE
    @Path("securitygroups/{id}")
    @Produces(MediaType.APPLICATION_JSON)
    SecurityGroupJson delete(@PathParam("id") Long id);

    @DELETE
    @Path("account/securitygroups/{name}")
    @Produces(MediaType.APPLICATION_JSON)
    SecurityGroupJson deletePublic(@PathParam("name") String name);

    @DELETE
    @Path("user/securitygroups/{name}")
    @Produces(MediaType.APPLICATION_JSON)
    SecurityGroupJson deletePrivate(@PathParam("name") String name);
}
